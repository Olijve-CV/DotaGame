...
The agent runtime currently exposes two search tools:

- `knowledge_search`: local RAG retrieval over the repository's indexed Dota knowledge
- `websearch`: an OpenCode-style explicit web search tool backed by Exa MCP

`websearch` posts to `https://mcp.exa.ai/mcp` by default. You can override its runtime settings with:
